...
class EditorPanel:
    def __init__(self, parent):
        ...
        self._line_nums = tk.Text(edit_frame,
                                   ...
                                   relief=tk.FLAT, bd=0,
                                   padx=0, pady=8,
                                   selectbackground=Theme.EDITOR_BG,
                                   cursor="arrow")
        self._line_nums.pack(side=tk.LEFT, fill=tk.Y)

        # İnce ayırıcı
        tk.Frame(edit_frame, bg=Theme.BORDER, width=1).pack(side=tk.LEFT, fill=tk.Y)
        ...
        self._editor.tag_configure("error",
                                    underline=True,
                                    underlinefg=Theme.RED,
                                    background="#2D1B1B")
        ...
        self.autocompleter = AutocompleteHelper(
            self._editor, self.frame, self._on_manual_completion)
    ...
    def _on_key_release(self, _event=None):
        ...
        if _event and _event.keysym in (
